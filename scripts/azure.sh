...
#
# Usage:
#   ./azure-setup.sh up [--hosting]  # create resources + print env vars
#                                    # --hosting also creates App Service plan + Web App
...
# Billing note: hosting (App Service plan + Web App) is NOT created by default.
#               Pass --hosting to `up` when you need to deploy to Azure.
...
# Whether to provision hosting (App Service plan + Web App).
# Default: off — saves billing cost for local dev.
# Enable with: ./azure.sh up --hosting
HOSTING=false
...
# Args: $1=storage conn, $2=AI key, $3=AI endpoint, $4=include_webapp (optional, default false)
write_env() {
  local conn="$1" key="$2" endpoint="$3" include_webapp="${4:-false}"
...
EOF
    if [[ "$include_webapp" == "true" ]]; then
      echo ""
      echo "# --- App ---"
      echo "AZURE_WEBAPP_URL=\"https://${WEBAPP}.azurewebsites.net\""
    fi
  } > "$ENV_FILE"
...
# ----------------------------------------------------------------------
# up — create core resources; pass --hosting to also create App Service
# ----------------------------------------------------------------------
up() {
  [[ "${1:-}" == "--hosting" ]] && HOSTING=true
  ...
  if [[ "$HOSTING" == "true" ]]; then
    echo ">> [hosting] Provisioning App Service plan and Web App"
    ...
    echo ">> [hosting] Skipped — run '$0 up --hosting' to provision App Service plan + Web App"
  fi

  echo ">> Writing local env file"
  write_env "$conn" "$key" "$endpoint" "$HOSTING"
...
info() {
  ...
  local has_webapp=false
  if az webapp show --name "$WEBAPP" --resource-group "$RG" -o none 2>/dev/null; then
    has_webapp=true
    echo "App URL        : https://${WEBAPP}.azurewebsites.net"
  else
    echo "App URL        : (no Web App provisioned — run '$0 up --hosting' to create one)"
  fi
  ...
  write_env "$conn" "$key" "$endpoint" "$has_webapp"
...
case "${1:-}" in
  ...
  info)   info ;;
  deploy) deploy ;;
  *)      echo "Usage: $0 {up [--hosting]|down [--wait]|info|deploy}"; exit 1 ;;
esac